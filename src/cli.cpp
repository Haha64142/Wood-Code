    ...
    if (command == "-h" || command == "--help" || command == "help") {
        ...
        if (argc == 3) {
            ...
            if (input == "--file" || input == "-f") {
                std::cout << "Warning: You entered 'woodcode encode " << input
                          << "' without specifying a file name.\n"
                          << "If you intended to encode a file, use: woodcode encode " << input
                          << " <file>\n\n"
                          << "Encoding '" << input << "' to WoodCode\n\n";
            }
        } else if (argc == 4 &&
                   (std::string(argv[2]) == "--file" || std::string(argv[2]) == "-f")) {
            input = readFile(argv[3]);
            if (input.empty()) {
                std::cerr << "The file you've provided is empty. If you want to encode an empty "
                             "string, use woodocode encode \"\"\n";
                return 1;
            }
            isFile = true;
        } else if (argc == 2) {
        ...
        if (argc == 3) {
            input = argv[2];
            if (input == "--file" || input == "-f") {
                std::cerr << "Error: You used option --file without specifying a file to use for "
                             "decoding.\n";
                return 1;
            }
        } else if (argc == 4 &&
